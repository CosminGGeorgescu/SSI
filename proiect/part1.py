...
for encrypted_file in os.listdir('encrypted_files'):
    n = math.ceil(math.sqrt(os.path.getsize(os.path.abspath('encrypted_files/' + encrypted_file))))
    for i in range(n):
        for j in range(n):
            if hashlib.sha256(f'P6 {i} {j} 255'.encode()).hexdigest() in hashes:
                sizes.add((i, j))
print(sizes)
